function __print_sigma_functions_help() {
cat <<EOF
Additional SigmaDroid functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- sigmagerrit:   A Git wrapper that fetches/pushes patch from/to SigmaDroid Gerrit Review.
- sigmarebase:   Rebase a Gerrit change and push it again.
- sigmaremote:   Add git remote for SigmaDroid Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for SigmaDroid Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Sigma model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch sigma_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/sigma-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.sigma.device | grep -q "$SIGMA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SIGMA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function sigmaremote()
{
    ...
    git remote rm sigma 2> /dev/null
    ...
    then
        ...
        local PFX="SigmaDroid-Project/"
    else
        local PROJECT=$REMOTE
    fi

    local SIGMA_USER=$(git config --get review.review.sigmadroid-project.org.username)
    if [ -z "$SIGMA_USER" ]
    then
        git remote add sigma ssh://review.sigmadroid-project.org:29418/$PFX$PROJECT
    else
        git remote add sigma ssh://$SIGMA_USER@review.sigmadroid-project.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'sigma' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/SigmaDroid-Project/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.sigma.device | grep -q "$SIGMA_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        sigmaremote
        git push sigma HEAD:refs/heads/'$1'
    fi
    '
}

function sigmagerrit() {
    ...
    local user=`git config --get review.review.sigmadroid-project.org.username`
...
function sigmarebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "SigmaDroid-Project Gerrit Rebase Usage: "
        echo "      sigmarebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.sigmadroid-project.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.sigma.device | grep -q "$SIGMA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/sigma/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SIGMA_FIXUP_COMMON_OUT ]; then
